...
class Oracle:
    ...
    def make_payload(self, userdata: bytes):
        ...
        ciphertext, _ = self._scheme.encrypt(self._make_payload(userdata), initialization_vector)
    ...
    def is_admin(self, payload: AESCBCOutput):
        plaintext = self._scheme.decrypt(payload.ciphertext, payload.initialization_vector)
...
def forge_admin(encrypt_func, check_func):
    ...
    new_payload = AESCBCOutput(ciphertext=bytearray(payload.ciphertext), initialization_vector=payload.initialization_vector)
    is_admin = False
    block_number = 0
    while not is_admin:
        block_slice = slice(block_number*block_size, (block_number + 1)*block_size)
        new_payload.ciphertext[block_slice] = crypto_base.xor_block(new_payload.ciphertext[block_slice], bit_difference)
