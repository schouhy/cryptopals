...
def test_pkcs7_pad_computes_value_correctly():
    ...
    assert (
        crypto_base.pkcs7_pad(plaintext1, block_size=16) == expected_padding_plaintext1
    )
    plaintext2 = b"A" * 16
    expected_padding_plaintext2 = b"A" * 16 + b"\x10" * 16
    assert (
        crypto_base.pkcs7_pad(plaintext2, block_size=16) == expected_padding_plaintext2
    )
...
def test_pkcs7_unpad_computes_value_correctly():
    ...
    assert (
        crypto_base.pkcs7_unpad(padded_plaintext, block_size=16) == expected_plaintext
    )
...
def test_aes_cbc_encrypts_correctly():
    ...
    expected_ciphertext = crypto_base.hex_to_bytes(
        ""
    )
    key = b"YELLOW SUBMARINE"
